---
title: "Multi-Level Modelling and Effect Statistics for Sports Scientists in R"
...
# Why sports scientist should be familiar with mixed effects (multi-level) modelling and effect statistics...
...
Multi-level models are quite useful in assessing longitudinal outcomes because of their ability to incorporate multiple factors into the model, include time-variant and time-invariant factors, account for between and within-athlete variability, handle missing and unbalanced data and account for repeated measures over time [(Windt et al. 2018)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6169745/).
...
data <- data %>% 
  ...
  left_join(data %>% 
              group_by(Name) %>%
              summarise(Count = n()), by = 'Name') %>%
  filter(Count >30) %>%
  mutate(Season = SeasonType) %>%
  mutate(PlayerID = group_indices(., Name)) %>%
  mutate(Athlete = paste0("Athlete", PlayerID)) %>% 
  dplyr::select(Athlete, Date, Season, Distance) %>%
  mutate(Season = as.factor(Season)) %>%
  mutate(Season = fct_relevel(Season, c("Preseason", "Inseason", "Postseason"))) %>%
